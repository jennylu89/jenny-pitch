...
function MetricCard({ metric, dark }) {
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 'var(--type-body)', color: dark ? 'var(--dark-text-muted)' : 'var(--muted)', lineHeight: 'var(--leading-body)',
      }}>{metric.label}</div>
    </div>
  );
}
...
function CaseStudySection({ section, index }) {
  ...
  return (
    <section ref={ref} style={{
      ...
      padding: 'var(--space-96) 0', position: 'relative', overflow: 'hidden',
    }}>
      {dark && <NoiseOverlay />}
      <div className="section-pad" style={{
        ...
        <p style={{
          fontSize: 'var(--type-body)', color: dark ? 'var(--dark-text-muted)' : 'var(--muted)',
          lineHeight: 'var(--leading-body)', margin: '0 auto var(--space-18)', maxWidth: '600px', textAlign: 'center',
        }}>{section.requirement}</p>
        <h2 style={{
          fontFamily: 'var(--font-sans)', fontSize: 'var(--type-h2)', fontWeight: 'var(--weight-medium)',
          lineHeight: 'var(--leading-h2)', letterSpacing: 'var(--tracking-h2)',
          color: dark ? 'var(--dark-text)' : 'var(--text)',
          margin: '0 auto var(--space-36)', maxWidth: '650px', textAlign: 'center',
        }}>{section.title}</h2>
        <div style={{
          display: 'flex', flexDirection: 'column', gap: 'var(--space-18)',
          maxWidth: '640px', margin: '0 auto var(--space-48)', textAlign: 'left',
        }}>
          {section.story.map((p, i) => (
            <p key={i} style={{
              fontSize: 'var(--type-body)', lineHeight: 'var(--leading-body)',
              color: dark ? 'var(--dark-text-muted)' : 'var(--muted)', margin: 0,
            }}>{p}</p>
          ))}
        </div>
        {section.metrics && (
          <div style={{
            ...
            gap: 'var(--space-12)', marginBottom: section.testimonial ? 'var(--space-48)' : 0,
            maxWidth: '480px', marginLeft: 'auto', marginRight: 'auto',
          }}>
            {section.metrics.map((m, i) => <MetricCard key={i} metric={m} dark={dark} />)}
          </div>
        )}
        {section.testimonial && (
          <div style={{
            borderLeft: `3px solid ${dark ? 'var(--accent-strong)' : 'var(--accent)'}`,
            paddingLeft: 'var(--space-24)', maxWidth: '540px', marginLeft: 'auto', marginRight: 'auto',
          }}>
            <p style={{
              fontFamily: 'var(--font-sans)', fontSize: 'var(--type-lead)', fontWeight: 'var(--weight-medium)',
              lineHeight: 'var(--leading-body)', color: dark ? 'var(--dark-text)' : 'var(--text)',
              margin: '0 0 var(--space-12)', fontStyle: 'italic',
            }}>"{section.testimonial.quote}"</p>
            <div style={{ fontSize: 'var(--type-body)', fontWeight: 'var(--weight-medium)', color: dark ? 'var(--dark-text)' : 'var(--text)' }}>
              {section.testimonial.name}
            </div>
            <div style={{ fontSize: 'var(--type-small)', color: dark ? 'var(--dark-text-muted)' : 'var(--muted)', marginTop: '2px' }}>
      ...
      <div style={{
        ...
        <div className="section-pad" style={{
          ...
          <div className="resume-cta-buttons" style={{ display: 'flex', gap: 'var(--space-18)', alignItems: 'center', marginTop: 'var(--space-24)' }}>
            ...
            <a className="btn-hover" href="/resume?from=mmc"
              style={{
